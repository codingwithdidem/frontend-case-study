...
export default function PackageSummary() {
        ...
        for (const pt of mockProducts) {
          for (const st of pt.subTypes) {
            if (st.id === sel.subTypeId) {
              ...
              const price = optimizations.reduce(
                (total, opt) => total + opt.product.price * opt.count,
                0,
              );
              return {
                subType: st,
  ...
  if (appliedPromo) {
    const promo = appliedPromo.promo;
    if (promo.type === "fixed") {
  ...
  return (
    <Paper elevation={2} sx={{ p: 4, width: "100%" }}>
      ...
      {summaryDetails.map((detail) => {
        ...
        return (
          <Box
            key={detail.subType.id}
            data-testid={`summary-row-${detail.subType.id}`}
            sx={{ mb: 2 }}
          >
            <Typography variant="subtitle1">
              {detail.subType.name} ({detail.quantity})
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {detail.optimizations
                .map((opt) => `${opt.count} x ${opt.product.size}li`)
                .join(" + ")}
            </Typography>
      ...
      {discount > 0 && (
        ...
        >
          <Typography>İndirim ({appliedPromo?.code})</Typography>
